...
public class TimelineGroupDataLoader implements PreparableReloadListener {
    ...
    public Map<ResourceLocation, JsonElement> prepare(ResourceManager resourceManager, ProfilerFiller profilerFiller) {
        ...
        // String entity = "bee";
        // EntityType<?> entityType = EntityType.byString(entity).isPresent() ?
        // EntityType.byString(entity).get() : null;
        ...
        for (ResourceLocation resourceLocation : passedFiles) {
            ...
            try {
                    ...
                    try {
                        ...
                        try {
                            ...
                            if (jsonElement != null) {
                                ...
                                ImprovedAnimations.LOGGER.error("Couldn't load data file {} as it's null or empty",
                                        resourceLocation);
                            }
                ...
                ImprovedAnimations.LOGGER
                        .error("Error parsing data upon grabbing resource for resourceLocation " + resourceLocation);
            }
        }
    ...
    protected void apply(Map<ResourceLocation, JsonElement> data, ResourceManager resourceManager,
        ...
        for (ResourceLocation resourceLocationKey : data.keySet()) {
            ...
            String resourceBody = resourceLocationKey.toString().split(":")[1].split("\\.")[0]
                    .replace("timelinegroups/", "");
            ResourceLocation finalResourceLocation = ResourceLocation.fromNamespaceAndPath(resourceNamespace,
                    resourceBody);

            // String entityKey = resourceLocationKey.toString().split("/")[1];
            // String animationKey =
            // resourceLocationKey.toString().split("/")[2].split("\\.")[0];
            ...
            if (Objects.equals(formatVersion, FORMAT_VERSION)) {
                ...
                for (int partIndex = 0; partIndex < partArrayJSON.size(); partIndex++) {
                    ...
                    for (Map.Entry<String, JsonElement> keyframeEntry : partKeyframesJSON.entrySet()) {
                        ...
                        channelTimeline.addKeyframe(TransformChannel.X, keyframeNumber,
                                keyframeJSON.getAsJsonObject().get("translate").getAsJsonArray().get(0).getAsFloat());
                        channelTimeline.addKeyframe(TransformChannel.Y, keyframeNumber,
                                keyframeJSON.getAsJsonObject().get("translate").getAsJsonArray().get(1).getAsFloat());
                        channelTimeline.addKeyframe(TransformChannel.Z, keyframeNumber,
                                keyframeJSON.getAsJsonObject().get("translate").getAsJsonArray().get(2).getAsFloat());

                        channelTimeline.addKeyframe(TransformChannel.X_ROT, keyframeNumber,
                                keyframeJSON.getAsJsonObject().get("rotate").getAsJsonArray().get(0).getAsFloat());
                        channelTimeline.addKeyframe(TransformChannel.Y_ROT, keyframeNumber,
                                keyframeJSON.getAsJsonObject().get("rotate").getAsJsonArray().get(1).getAsFloat());
                        channelTimeline.addKeyframe(TransformChannel.Z_ROT, keyframeNumber,
                                keyframeJSON.getAsJsonObject().get("rotate").getAsJsonArray().get(2).getAsFloat());

                        /*
                         * for(Map.Entry<String, JsonElement> attributeEntry :
                         * keyframeJSON.getAsJsonObject().entrySet()){
                         * TransformChannel transformChannel =
                         * TransformChannel.valueOf(attributeEntry.getKey());
                         * float keyframeValue = attributeEntry.getValue().getAsFloat();
                         * 
                         * channelTimeline = channelTimeline.addKeyframe(transformChannel,
                         * keyframeNumber, keyframeValue);
                         * //AnimationOverhaul.LOGGER.info("Channel: {} Value: {}", transformChannel,
                         * keyframeValue);
                         * }
                         */
                    }
                ...
                // AnimationOverhaul.LOGGER.info("Entity key: {} Animation key: {}", entityKey,
                // animationKey);

                ImprovedAnimations.LOGGER.info("Successfully loaded animation {}", resourceLocationKey);
            } else {
                ImprovedAnimations.LOGGER.error(
                        "Failed to load animation {} (Animation format version was {}, not up to date with {})",
                        resourceLocationKey, formatVersion, FORMAT_VERSION);
            }
        }
    ...
    public CompletableFuture<Void> reload(PreparationBarrier preparationBarrier, ResourceManager resourceManager,
        ...
        return CompletableFuture.supplyAsync(() -> this.prepare(resourceManager, profilerFiller), executor)
                .<Object>thenCompose(preparationBarrier::wait).thenAcceptAsync(object -> this
                        .apply((Map<ResourceLocation, JsonElement>) object, resourceManager, profilerFiller2),
                        executor2);
    }
}
